#ifndef __ASMJIT_PATTERN_MATCH__
#define __ASMJIT_PATTERN_MATCH__
...
struct AsmJitPattern
{
    virtual void emit(AsmJitCodeGen* pCodeGen) = 0;
    virtual ~AsmJitPattern() {}
};

struct x86_PM_SDAG
{
    x86_PM_SDAG(AsmJitPattern* pattern, llvm::Instruction *pRoot) : m_pPattern(pattern), m_pRoot(pRoot)
    {

    }

    AsmJitPattern*           m_pPattern;
    llvm::Instruction*    m_pRoot;
};

struct AsmJit_PM_BasicBlock
{
    unsigned int id;
    llvm::BasicBlock* pBB;
...
struct x86_PM_FuncBasicBlock
{
    ...
    std::vector<AsmJit_PM_BasicBlock*> m_blocks;
    llvm::DenseMap<llvm::BasicBlock*, AsmJit_PM_BasicBlock*> m_blockMap;

    llvm::Function *m_pFunction;

    void addBlock(unsigned int id, llvm::BasicBlock* pBB, AsmJit_PM_BasicBlock* pPMBB)
    {
        pPMBB->id = id;
        pPMBB->pBB = pBB;
...
class AsmJitPatternMatch : public llvm::ModulePass, public llvm::InstVisitor<AsmJitPatternMatch>
{
public:
...
private:
    ...
    AsmJitPattern* match(llvm::Instruction *pI);
    ...
    void addPattern(AsmJitPattern *pPattern)
    {
...
private:
    ...
    llvm::Instruction*                                         m_pRoot;
    AsmJitPattern*                                             m_pCurrentPattern;
    std::vector<AsmJitPattern*>                                m_patterns;
};
