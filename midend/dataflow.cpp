...
//
void DataFlowPass::computeGenKill(const llvm::Function& fn, BlockStates& states)
{
    for (llvm::Function::const_iterator FI = fn.begin(), FE = fn.end(); FI != FE; ++FI)
    {
        const llvm::BasicBlock& block = *FI;
        BlockState state;
...
//
void DataFlowPass::traverseForwards(const llvm::Function& fn, BlockStates& states)
{
    ...
    const llvm::BasicBlock* start = &(fn.front());
    ...
    while (!worklist.empty())
    {
        ...
        for (llvm::const_pred_iterator I = pred_begin(current), IE = pred_end(current);
            I != IE; ++I)
        {
        ...
        for (llvm::succ_const_iterator I = succ_begin(current), IE = succ_end(current);
            I != IE; ++I)
        {
            worklist.push(*I);
        }
...
// analysis. The BlockState map is updated during the traversal.
//
void DataFlowPass::traverseBackwards(const llvm::Function& fn, BlockStates& states)
{
    std::queue<const llvm::BasicBlock*> worklist;
    std::set<const llvm::BasicBlock*> visited;
    ...
    // Set up initial conditions.
    Assignments top = getTop(fn);
    const llvm::BasicBlock* start = &(fn.back());
    BlockState& start_state = states[start];
    start_state.out = top;
    worklist.push(start);
...
//
Assignments DataFlowPass::getTop(const llvm::Function& fn)
{
    if (_top == ALL)
    {
...
//
void DataFlowPass::display(const llvm::Function& fn, BlockStates& states)
{
    std::cout << "Function: " << fn.getName().data() << std::endl << std::endl;
...
//
bool DataFlowPass::runOnFunction(llvm::Function& fn)
{
...
void DataFlowPass::getAnalysisUsage(llvm::AnalysisUsage& AU) const
{
    AU.setPreservesCFG();
}
